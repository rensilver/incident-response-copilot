...
Replace the entire contents of `tests/unit/evaluation/test_eval_runner.py` with (this
adds the new imports and tests while keeping every test from Task 3 - it replaces the
whole file, not an append, so the import block stays one sorted group):
...
Replace the entire contents of `tests/unit/evaluation/test_eval_runner.py` with (Task
4's file plus the new `pytest` import merged into the top block, `_print_results`
merged into the existing `eval_runner` import, and one new test appended at the end -
full-file replacement again, same reason as Task 4):
